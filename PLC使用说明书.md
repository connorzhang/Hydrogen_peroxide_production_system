...
| `Q0.2` | 纯水机/制水机 | 任一桶缺水即开启，双桶满则停 |
| `Q0.3` | 空压机（接触器/电磁阀） | 关机后延时 30 分钟关闭 |
| `Q0.4` | 阳极控制电磁阀 | 空压机后延时开启 |
| `Q0.5` | 阴极液体电磁阀 | 阴极泵后延时开启 |
| `Q0.6` | 直流电源开关控制 | 阴极阀开启 5 分钟后使能 |
...
graph LR
  ...
  CPU --> RO["制水机<br/>Q0.2"]:::act
  CPU --> Air["空压机<br/>Q0.3"]:::act
  CPU --> V3["阳极阀<br/>Q0.4"]:::act
  CPU --> V4["阴极阀<br/>Q0.5"]:::act
  CPU --> DC["直流电源<br/>Q0.6"]:::act
...
graph LR
  ...
  P["压力<br/>AIW16"] --> AE08["AE08<br/>AIW16~AIW30"]
  LA["阳极液位<br/>AIW18"] --> AE08
  LC["阴极液位<br/>AIW20"] --> AE08
  AE08 --> CPU
...
传感器：AIW16(压力) / AIW18(阳极液位) / AIW20(阴极液位)

PLC ST30 + AE08 + AQ02
  DO：Q0.0 Q0.1 Q0.2 Q0.3 Q0.4 Q0.5 Q0.6  -> 阀/空压机/制水机/直流电源
...
| 一直在补水不停 | `AIW18/AIW20` 是否有数值变化；液位阈值是否过高 |
| 阳极泵不转 | 泵驱动方式（PWM/模拟量给定）是否接对；`AQW32` 或 PWM 输出是否有信号 |
| 阴极阀不打开 | `Q0.5` 是否输出；阴极泵是否已启动并完成 3 秒延时 |
| 关机后空压机不延时 | `Q0.6` 是否被其他逻辑控制；是否等待满 30 分钟 |
